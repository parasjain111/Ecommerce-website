...
import {Catapi,prodapi,getsingleproducts,updateproduct,getcategories} from './adminapi';
...
const first=(productId)=>{
    ...
    .then(data=>{
        if(data.error){
            ...
            setvalues({...values,name:data.name,description:data.description,price:data.price,category:data.category,shipping:data.shipping,quantity:data.quantity,formData:new FormData()})
            firstcategory();
        }
    })
}
...
useEffect(()=>{
    ...
    first(props.match.params.productId);
},[])
